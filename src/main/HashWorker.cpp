...
void HashWorker::start() {
	...
	LOG4CPLUS_INFO(logger, "Hashed " << db.getRecordsWritten() << " of " << totalNumOfFiles << " images, sha-lookup:"<< db.get_sha_found() << ", skipped " << db.get_skipped_files() << ", invalid: " << db.get_invalid_files());
}
